...
export default function IngredientItemGaro({ item , user ,  stat }){
    ...
    return(
        ...
        stat ? 
        <li key={item.id} className='bg-sky-300 w-32 h-14 rounded-xl flex flex-row items-center my-2' onClick={()=>{
        ...
        :
        <li key={item.id} className='-bg--fridge-gray w-32 h-14 rounded-xl flex flex-row items-center my-2' onClick={()=>{
            console.log('stat : ', stat);
            handle(stat);
        }}>
            <figure>
